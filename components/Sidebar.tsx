...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setCurrentView, openLearnModal, isOpen, setIsOpen }) => {
  ...
  const { targetId, setTargetId } = useStrategicTarget();
                ...
                return (
                    <div key={p.id} className="group w-full flex items-center justify-between p-3 rounded-lg text-left text-light-text-secondary dark:text-dark-text-secondary hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-brand-primary">
                        ...
                        <div className="flex items-center">
                            <button onClick={() => setTargetId(isTarget ? null : p.id)} className={`p-1 rounded-full ${isTarget ? 'text-brand-primary' : 'text-gray-400 hover:text-brand-accent'}`} title={isTarget ? "This is your strategic target" : "Set as strategic target"}>
                                <FlagIcon filled={isTarget} />
                            </button>
                            <button onClick={() => deletePortfolio(p.id)} className="opacity-0 group-hover:opacity-100 text-red-500 hover:text-red-700 ml-1">
                                <DeleteIcon />
                            </button>
                        </div>
                    </div>
...
const FlagIcon: React.FC<{ filled: boolean }> = ({ filled }) => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill={filled ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
        <path strokeLinecap="round" strokeLinejoin="round" d="M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H17a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2z" />
    </svg>
);
