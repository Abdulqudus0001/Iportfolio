...
interface Asset {
  ...
  asset_class: 'EQUITY' | 'CRYPTO' | 'BENCHMARK' | 'ETF'; price?: number; is_shariah_compliant?: boolean;
...
interface BlackLittermanView {
    id: string; asset_ticker_1: string; direction: 'outperform' | 'underperform';
...
const withCache = async <T>(key: string, fetcher: () => Promise<T>, ttlSeconds: number): Promise<ServiceResponse<T>> => {
  ...
  const { data: cachedData, error: cacheError } = await supabaseAdmin.from('api_cache').select('data, last_fetched').eq('key', key).single();
  if (cacheError && cacheError.code !== 'PGRST116') console.error(`Cache read error for key ${key}:`, cacheError.message);

  if (cachedData) {
    const isStale = new Date().getTime() - new Date(cachedData.last_fetched).getTime() > ttlSeconds * 1000;
    if (!isStale) return { data: cachedData.data as T, source: 'cache' };
  }

  try {
    const liveData = await fetcher();
    const { error: upsertError } = await supabaseAdmin.from('api_cache').upsert({ key, data: liveData, last_fetched: new Date().toISOString() });
    if (upsertError) console.error(`Cache write error for key ${key}:`, upsertError.message);
    return { data: liveData, source: 'live' };
  } catch(error) {
     if (cachedData) {
      console.warn(`Live fetch for ${key} failed. Serving stale data. Error: ${error.message}`);
      return { data: cachedData.data as T, source: 'cache' };
    }
    console.error(`Live fetch for ${key} failed and no cache is available.`, error);
    throw error;
  }
};
...
const invert = (matrix: number[][]): number[][] => {
    ...
    for (let i = 0; i < n; i++) {
        let pivot = i; while (pivot < n && augmented[pivot][i] === 0) pivot++;
        if (pivot === n) throw new Error("Matrix is singular and cannot be inverted.");
        [augmented[i], augmented[pivot]] = [augmented[pivot], augmented[i]];
        const divisor = augmented[i][i]; for (let j = i; j < 2 * n; j++) augmented[i][j] /= divisor;
        for (let k = 0; k < n; k++) if (k !== i) { const factor = augmented[k][i]; for (let j = i; j < 2 * n; j++) augmented[k][j] -= factor * augmented[i][j]; }
    }
...
const calculatePortfolioMetrics = (weights: number[], meanReturns: number[], covMatrix: number[][]) => {
    ...
    const variance = multiply([weights], multiply(covMatrix, transpose([weights])))[0][0];
...
const handlers: Record<string, (payload: any) => Promise<any>> = {
  getAvailableAssets: async () => withCache('available-assets-v8-etfs', async () => {
    const etfTickers = ['BND', 'GLD', 'VNQ'];
    const etfPromise = apiFetch(`${FMP_BASE_URL}/profile/${etfTickers.join(',')}?apikey=${FMP_API_KEY}`)
        .then(data => (Array.isArray(data) ? data : []).map((e: any) => {
            let sector = 'ETF';
            if(e.symbol === 'BND') sector = 'Fixed Income';
            if(e.symbol === 'GLD') sector = 'Commodities';
            if(e.symbol === 'VNQ') sector = 'Real Estate';
            return {
                ticker: e.symbol, name: e.companyName, country: 'US', sector,
                asset_class: 'ETF', price: e.price, liquidity: 'High'
            } as Asset;
        }));
    ...
    const screenerPromise = apiFetch(`${FMP_BASE_URL}/stock-screener?marketCapMoreThan=1000000000&volumeMoreThan=10000&limit=100&apikey=${FMP_API_KEY}`)
        .then(data => (Array.isArray(data) ? data : []).map((s: any) => {
            let liquidity: Asset['liquidity']; const mc = s.marketCap;
            if (mc > 10e9) liquidity = 'High'; else if (mc > 2e9) liquidity = 'Medium'; else if (mc > 0) liquidity = 'Low';
            return {
                ticker: s.symbol, name: s.companyName, country: 'US',
                sector: s.sector || 'N/A', asset_class: 'EQUITY', price: s.price, liquidity
            } as Asset;
        }));

    const majorCryptoTickers = ['BTC', 'ETH', 'BNB', 'SOL', 'XRP', 'ADA', 'DOGE', 'AVAX', 'SHIB', 'DOT'];
    const cryptoPromise = apiFetch(`${FMP_BASE_URL}/quote/${majorCryptoTickers.map(t => `${t}USD`).join(',')}?apikey=${FMP_API_KEY}`)
        .then(data => (Array.isArray(data) ? data : []).map((c: any) => ({
            ticker: c.symbol.replace(/USD$/, ''), name: c.name, country: 'CRYPTO',
            sector: 'Cryptocurrency', asset_class: 'CRYPTO', price: c.price, liquidity: 'High'
        } as Asset)));

    const [etfResult, screenerResult, cryptoResult] = await Promise.allSettled([etfPromise, screenerPromise, cryptoPromise]);

    const combinedAssets = new Map<string, Asset>();
    (etfResult.status === 'fulfilled' ? etfResult.value : []).forEach(asset => combinedAssets.set(asset.ticker, asset));
    (screenerResult.status === 'fulfilled' ? screenerResult.value : []).forEach(asset => combinedAssets.set(asset.ticker, asset));
    (cryptoResult.status === 'fulfilled' ? cryptoResult.value : []).forEach(asset => combinedAssets.set(asset.ticker, asset));
  ...
  getAssetPriceHistory: async ({ ticker }) => withCache(`price-history-${ticker}`, async () => {
    const {data: {assets: allAssets}} = await handlers.getAvailableAssets({});
    ...
    throw new Error(`No historical price data could be found for ${ticker}.`);
  }, TTL_60_DAYS),
  ...
  getAssetPriceSummary: async ({ ticker }) => withCache(`price-summary-${ticker}`, async () => {
    ...
    const quote = data[0];
    if (quote && quote.price != null) return { open: quote.open ?? 0, close: quote.price, high: quote.dayHigh ?? 0, low: quote.dayLow ?? 0, volume: formatVolume(quote.volume) };
    throw new Error(`Could not retrieve price summary for ${ticker}.`);
  }, TTL_15_MINUTES),

  getFinancialRatios: async ({ ticker }) => withCache(`ratios-${ticker}`, async () => {
    const {data: {assets: allAssets}} = await handlers.getAvailableAssets({});
    const assetInfo = allAssets.find((a: Asset) => a.ticker === ticker);
    if (assetInfo?.asset_class !== 'EQUITY') return [];
    try {
        ...
        return finalRatios;
    } catch (e) { console.warn(`Could not fetch financial ratios for ${ticker}. Error: ${e.message}`); return []; }
  }, TTL_60_DAYS),

  getFinancialsSnapshot: async ({ ticker }) => withCache(`financials-${ticker}`, async () => {
    const {data: {assets: allAssets}} = await handlers.getAvailableAssets({});
    const assetInfo = allAssets.find((a: Asset) => a.ticker === ticker);
    if (assetInfo?.asset_class !== 'EQUITY') return { income: [], balanceSheet: [], cashFlow: [], asOf: 'N/A' };
    try {
        ...
        const result = { income: [] as {metric: string, value: string}[], balanceSheet: [] as {metric: string, value: string}[], cashFlow: [] as {metric: string, value: string}[], asOf: income.date || 'N/A' };
        ...
        return result;
    } catch (e) { console.warn(`Could not fetch financials for ${ticker}. Error: ${e.message}`); return { income: [], balanceSheet: [], cashFlow: [], asOf: 'N/A' }; }
  }, TTL_60_DAYS),

  getCompanyProfile: async ({ ticker }) => withCache(`profile-${ticker}`, async () => { const data = await apiFetch(`${FMP_BASE_URL}/profile/${ticker}?apikey=${FMP_API_KEY}`); const profile = data[0] || {}; return { description: profile.description, beta: profile.beta }; }, TTL_60_DAYS),
  getDividendInfo: async ({ ticker }) => withCache(`dividend-${ticker}`, async () => {
    const results = await Promise.allSettled([ apiFetch(`${FMP_BASE_URL}/historical-price-full/stock_dividend/${ticker}?apikey=${FMP_API_KEY}`), apiFetch(`${FMP_BASE_URL}/quote/${ticker}?apikey=${FMP_API_KEY}`) ]);
    if (results[0].status === 'rejected') return null;
    const dividendData = results[0].value; const quoteData = results[1].status === 'fulfilled' ? results[1].value : [];
    const historical = dividendData.historical || dividendData; if (!historical || historical.length === 0) return null;
    const lastDividend = historical[0]; const quote = quoteData[0] || {}; const yieldValue = quote.dividendYield || ((lastDividend.dividend * 4) / quote.price);
    return { ticker, yield: yieldValue, amountPerShare: lastDividend.dividend, payDate: lastDividend.paymentDate, projectedAnnualIncome: 0 };
  }, TTL_60_DAYS),
  ...
  generateAndOptimizePortfolio: async ({ template, optimizationModel, runner, constraints }) => {
    ...
    if (!availableAssets || availableAssets.length === 0) throw new Error("Could not fetch available assets.");
    let selectedAssets: Asset[] = [];
    
    switch (template) {
        case 'Aggressive': {
            const growthSectors = ['Technology', 'Communication Services', 'Consumer Cyclical'];
            const growthStocks = availableAssets.filter(a => a.asset_class === 'EQUITY' && growthSectors.includes(a.sector)).slice(0, 10);
            const topCryptos = availableAssets.filter(a => a.asset_class === 'CRYPTO').slice(0, 5);
            selectedAssets = [...growthStocks, ...topCryptos];
            break;
        }
        case 'Balanced': {
            const equities = availableAssets.filter(a => a.asset_class === 'EQUITY').slice(0, 15);
            const bondEtf = availableAssets.find(a => a.ticker === 'BND');
            selectedAssets = bondEtf ? [...equities, bondEtf] : equities;
            break;
        }
        default: throw new Error(`Invalid or unsupported portfolio template: ${template}`);
    }
    ...
    if (runner === 'optimize') {
        ...
        const result: OptimizationResult = { weights: validAssets.map((a, i) => ({ ...a, weight: equalWeights[i] })), ...metrics, currency: 'USD' };
    ...
    for (let i = 0; i < 7500; i++) {
        ...
        if (constraints?.maxSectorWeight) { const sectorWeights: Record<string, number> = {}; validAssets.forEach((asset, i) => { sectorWeights[asset.sector] = (sectorWeights[asset.sector] || 0) + weights[i]; }); if (Object.values(sectorWeights).some(sw => sw > constraints.maxSectorWeight)) continue; }
    ...
    const bestSharpeResult: OptimizationResult = { weights: validAssets.map((a, i) => ({ ...a, weight: optimal.weights[i] })), returns: optimal.returns, volatility: optimal.volatility, sharpeRatio: optimal.sharpeRatio, currency: 'USD' };
  ...
  runBlackLittermanOptimization: async ({ assets, views, currency }: { assets: Asset[], views: BlackLittermanView[], currency: string }) => {
    ...
    views.forEach((view, i) => { const index1 = assetIndexMap.get(view.asset_ticker_1), index2 = assetIndexMap.get(view.asset_ticker_2); if (index1 !== undefined) P[i][index1] = view.direction === 'outperform' ? 1 : -1; if (index2 !== undefined) P[i][index2] = view.direction === 'outperform' ? -1 : 1; });
    const Omega = multiply(P, multiply(scale(covMatrix, tau), transpose(P))).map((row, i) => row.map((val, j) => (i === j ? val / (views[i].confidence || 0.5) : 0)));
    const term1_inv = add(invert(scale(covMatrix, tau)), multiply(transpose(P), multiply(invert(Omega), P))); const term1 = invert(term1_inv);
    const term2 = add(multiply(invert(scale(covMatrix, tau)), transpose([equilibriumReturns])), multiply(transpose(P), multiply(invert(Omega), transpose([Q]))));
    ...
    for (let i = 0; i < 7500; i++) {
        const rand = Array.from({ length: numAssets }, () => Math.random()); const total = rand.reduce((a, b) => a + b, 0); if (total === 0) continue;
        const weights = rand.map(r => r / total);
        const { returns, volatility, sharpeRatio } = calculatePortfolioMetrics(weights, posteriorReturns, covMatrix);
        simulations.push({ returns, volatility, sharpeRatio });
        if (sharpeRatio > bestSharpePortfolio.sharpeRatio) bestSharpePortfolio = { returns, volatility, sharpeRatio, weights };
    }
    const bestSharpeResult: OptimizationResult = { weights: validAssets.map((a, i) => ({ ...a, weight: bestSharpePortfolio.weights[i] })), returns: bestSharpePortfolio.returns, volatility: bestSharpePortfolio.volatility, sharpeRatio: bestSharpePortfolio.sharpeRatio, currency: currency || 'USD' };
    return { bestSharpe: bestSharpeResult, simulations, averageWeights: bestSharpeResult.weights };
  },

  projectGoal: async({ goal, portfolio, annualContribution }: { goal: FinancialGoal, portfolio: OptimizationResult, annualContribution: number }) => {
    const yearsRemaining = (new Date(goal.targetDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24 * 365);
    if (yearsRemaining <= 0) return { successProbability: goal.currentAmount >= goal.targetAmount ? 1 : 0 };
    const monthlyReturn = portfolio.returns / 12, monthlyVolatility = portfolio.volatility / Math.sqrt(12);
    const monthlyContribution = annualContribution / 12;
    const months = Math.floor(yearsRemaining * 12);
    let successCount = 0; const numSimulations = 1000;
    for (let i = 0; i < numSimulations; i++) {
        let simValue = goal.currentAmount;
        for (let j = 0; j < months; j++) {
            const u1 = Math.random(), u2 = Math.random();
            const z = Math.sqrt(-2 * Math.log(u1)) * Math.cos(2 * Math.PI * u2);
            const monthlyGrowth = Math.exp(monthlyReturn - (monthlyVolatility**2)/2 + monthlyVolatility*z);
            simValue = simValue * monthlyGrowth + monthlyContribution;
        }
        if (simValue >= goal.targetAmount) successCount++;
    }
    return { successProbability: successCount / numSimulations };
  },
  ...
  calculateVaR: async ({ portfolio }) => {
    ...
    const var95 = portfolioDailyReturns[var95Index] ? -portfolioDailyReturns[var95Index] * portfolioValue : 0;
...
serve(async (req) => {
  ...
  try {
    ...
    if (result instanceof ReadableStream) return new Response(result, { headers: { ...corsHeaders, 'Content-Type': 'text/plain; charset=utf-8' } });
